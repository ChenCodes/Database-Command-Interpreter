...
class CommandInterpreter {
    ...
    void loadStatement() {
        _input.next("load");
        String name = name();
        Table stored = Table.readTable(name);
        _database.put(name, stored);
    ...
    Table tableDefinition() {
        String name = name();
        List<String> tableConstr = new ArrayList<String>();
        Table table = new Table(tableConstr);
        List<String> tableConstruct = new ArrayList<String>();

        if (_input.nextIf("(")) {
            String name1 = name();
            if (!tableConstruct.contains(name1)) {
                tableConstruct.add(name1);
            }
            while (_input.nextIf(",")) {
                name1 = name();
                if (tableConstruct.contains(name1)) {
            ...
            table = new Table(tableConstruct);
            _input.next(")");
        } else {
            _input.next("as");
            table = selectClause();
    ...
    Table selectClause() {
        ...
        if (_input.nextIf(",")) {
            ...
            allTables.add(otherTable);
        }
        for (String column : columnNames) {
            if (allTables.size() == 1) {
                if (myTable.findColumn(column) == -1) {
                    throw error("unknown column: %s", column);
                }
            }
        }
        if (myTable == otherTable) {
            return myTable.select(columnNames, null);
        }
        List<Condition> listConditions1 = null;
        if (otherTable == null) {
            listConditions1 = conditionClause(myTable);
        }
        if (listConditions1 == null && (allTables.size() == 1)) {
            return myTable.select(columnNames, null);
        } else if (listConditions1 != null && (allTables.size()) == 1) {

            return myTable.select(columnNames, listConditions1);
        } else if (listConditions1 == null && (allTables.size() == 2)) {
            Table finalTable = myTable.select(otherTable, columnNames, null);
            listConditions1 = conditionClause(finalTable);
            return finalTable.select(columnNames, listConditions1);
        }
        return null;
    }
    ...
    ArrayList<Condition> conditionClause(Table... tables) {
        ...
        if (_input.peek().equals("where")) {
            _input.next("where");
            listConditions.add(condition(tables));
    ...
    Condition condition(Table... tables) {
        ...
        String literalORCol = _input.peek();
        char firstLetter = literalORCol.charAt(0);
        String val2 = null;
        Column col2 = null;
        ...
        if (val2 != null) {
            newCondition = new Condition(col1, relation, val2);
        } else {

            col2 = new Column(columnName(), tables);
            newCondition = new Condition(col1, relation, col2);
        }
        return newCondition;
    }
